...
Complete syntax for searching issues in YouTrack. The query language is case-insensitive and uses `attribute: value`
syntax for structured search.
...
`has: <attribute>` returns issues containing a value for the attribute. `has: -<attribute>` returns issues with empty
values.

Supported with: `attachments`, `boards`, `Board <name>`, `comments`, `description`, `<field name>`, `Gantt`,
`<link type>`, `links`, `star`, `votes`, `vcs changes`, `work`.
...
Default resolved states: Fixed, Won't fix, Duplicate, Incomplete, Obsolete, Can't reproduce. Default unresolved states:
Submitted, Open, In Progress, Reopened, To be discussed.
...
- `( )` — group conditions, processed first. All operators joining parenthetical statements to neighbors must be
  explicit.
