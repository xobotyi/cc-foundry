...
Detailed conventions for interactive and non-interactive modes, prompts, confirmation dialogs, progress display, and
user input. Extends the behavioral rules in SKILL.md with patterns and edge cases.

## TTY Detection

The fundamental mechanism for deciding between interactive and non-interactive behavior is checking whether
stdin/stdout/stderr are connected to a terminal (TTY).
...
Check each stream independently. A common pattern is stdout piped to another program while stderr still shows progress
in the terminal.
...
- Never prompt when stdin is not a TTY -- fail with an error telling the user which flag to pass.
...
- Prompt with `y/N` (default no), or skip if clearly intentional (e.g., `rm` implies deletion)
...
4. Group similar errors under a single header rather than printing many similar lines.
5. For unexpected errors, include debug info and instructions for filing a bug report.
6. Don't show stack traces by default -- put them behind `--verbose` or write to a log file.

## Letting Users Escape

- Make Ctrl-C work at all times, even during network I/O.
- If your program wraps another process where Ctrl-C doesn't propagate, document the escape sequence.
