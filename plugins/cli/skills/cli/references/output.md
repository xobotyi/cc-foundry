...
Detailed conventions for stdout, stderr, structured output, color, and terminal detection. Extends the behavioral rules
in SKILL.md with full patterns and edge cases.

## Stream Separation

### stdout: Data Only

stdout carries the program's primary data output -- the content that gets piped to the next command or redirected to a
file.
...
When human-readable output breaks machine parseability (e.g., wrapped table cells), provide `--plain` for simple tabular
output:
...
- Print something within 100ms. If making a network request, say so before starting.
...
- Direct progress indicators to stderr so they don't interfere with piped stdout.
...
- `less -FIRX`: no paging if content fits one screen, case-insensitive search, color passthrough, leaves content on
  screen after quit
...
- Use color intentionally: highlight important information, distinguish errors, aid scannability.
...
1. **stdout/stderr is not a TTY.** Check each stream independently -- if piping stdout but stderr is still a terminal,
   keep stderr colors.
2. **`NO_COLOR` is set and non-empty** (regardless of value). This is the [no-color.org](https://no-color.org/)
   standard.
...
Support `FORCE_COLOR` or `--color` to force color output even when the above conditions would disable it. User-level
config and per-instance flags override `NO_COLOR`.
...
> variable that, when present and not an empty string (regardless of its value), prevents the addition of ANSI color.
...
Use a color library rather than hand-coding escape sequences. Libraries handle detection, NO_COLOR, and cross-platform
differences.
