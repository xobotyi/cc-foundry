...
Detailed conventions for process exit codes and signal-related exit behavior. Extends the behavioral rules in SKILL.md
with full tables and edge cases.
...
Scripts use `$?` or `set -e` to check exit codes. A program that returns 0 on partial failure will silently break
pipelines.
