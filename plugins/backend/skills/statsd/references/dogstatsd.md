...
DogStatsD is Datadog's extension of the StatsD protocol. It adds tags, histograms, distributions, service checks, and
events. Any compliant StatsD client works with DogStatsD for basic metrics, but the extensions require Datadog client
libraries.
...
- **`<METRIC_NAME>`** (required) — ASCII alphanumerics, underscores, periods
- **`<VALUE>`** (required) — Integer or float
- **`<TYPE>`** (required) — `c`, `g`, `ms`, `h`, `s`, `d`
- **`@<SAMPLE_RATE>`** (optional) — Float 0-1. Works with `c`, `h`, `d`, `ms`
- **`#<TAGS>`** (optional) — Comma-separated `key:value` pairs

## Tags

Tags are the primary advantage of DogStatsD over plain StatsD. They allow multi-dimensional queries without encoding
dimensions in the metric name.
...
1. **Use tags for queryable dimensions.** If you will filter or group by a value in dashboards, it should be a tag.
2. **Keep cardinality bounded.** Each unique tag combination creates a separate time series. Unbounded tags (user IDs,
   request IDs) explode custom metric counts and backend costs.
3. **Prefer lowercase.** Tags are case-sensitive in Datadog. `Method:GET` and `method:GET` are different tags.
...
Distributions send raw values to Datadog for server-side aggregation. This provides globally accurate percentiles across
all hosts.
...
**Produced metrics:** `sum`, `count`, `avg`, `min`, `max`, `p50`, `p75`, `p90`, `p95`, `p99`.
...
- **`<TEXT>`** (required) — Event body (use `\\n` for newlines)
- **`d:<TIMESTAMP>`** (optional) — Unix epoch (default: now)
- **`h:<HOSTNAME>`** (optional) — Hostname override
- **`p:<PRIORITY>`** (optional) — `normal` or `low` (default: `normal`)
- **`t:<ALERT_TYPE>`** (optional) — `error`, `warning`, `info`, `success`
...
- **v1.0** (all agents) — Base protocol with tags
...
**Timestamps (v1.3):** Pre-aggregated metrics can include a Unix timestamp to skip Agent-side aggregation:
