...
**Strict types, explicit contracts, no magic. If a class needs a docblock to explain what
its properties do, the properties are named wrong.**
...
- **Descriptive names.** `$userCount` not `$n`. Short names (`$i`, `$k`, `$v`) only in
...
- **Abbreviations as words.** `HttpClient` not `HTTPClient`, `JsonParser` not `JSONParser`.
...
- **Short type names:** `bool`, `int`, `float`, `string`. Never `boolean`, `integer`,
  `double`.
...
- **Avoid `mixed`** — it disables type safety. Use `object` when you mean "any object."
...
- **Backed enums** (`string` or `int`) when the value must interoperate with external
...
- **Asymmetric visibility** (8.4+) — `public protected(set)` for publicly readable,
  internally writable properties.
- **Property hooks** (8.4+) — `get`/`set` logic on properties. Use instead of trivial
...
- **`super()` equivalent:** always use `parent::method()`. Never hardcode grandparent
  class names.

### Magic Methods

- **Avoid property overloading** (`__get`, `__set`) in new code. Typed properties with
...
- **`match` over `switch`** — `match` is an expression, uses strict comparison, and
...
- **Arrow functions** for short closures: `fn($x) => $x * 2`. Arrow functions capture
  by value, not by reference.
- **Closures** for multi-statement callbacks. Use `use` to capture outer variables.
...
- **Be specific.** Catch the narrowest exception type: `catch (InvalidArgumentException)`
  not `catch (Exception)`.
...
- **Chain exceptions.** `throw new AppException('context', previous: $e)` preserves the
  original cause.
- **Custom exception hierarchy:**
  ...
  ```
- **Prefer `\RuntimeException` subtree** for application errors. `\LogicException` subtree
...
- **`str_contains()`, `str_starts_with()`, `str_ends_with()`** (8.0+) — never `strpos`
...
- **Exhaustiveness** — always include `default` unless provably exhaustive. Unmatched
  value throws `UnhandledMatchError`.

## Closures and Callables

- **First-class callable syntax** (8.1+): `strlen(...)`, `$obj->method(...)`,
  `ClassName::method(...)`.
- **Arrow functions** (`fn`) for single-expression closures — auto-captures by value.
- **Static closures** (`static fn`, `static function`) when `$this` is not needed —
...
- **`new Foo()`** — always use parentheses when instantiating (even without arguments),
  unless immediately chaining: `new Foo()->method()`.
- **Compound types:** no spaces around `|` and `&`. Parentheses for DNF without internal
  spaces.
- **Empty exception classes** on one line: `class NotFoundException extends AppException {}`

## Application

When **writing** PHP code: apply all conventions silently — don't narrate each rule. If an
existing codebase contradicts a convention, follow the codebase and flag the divergence once.

When **reviewing** PHP code: cite the specific violation and show the fix inline. Don't
...
markers, config values, env vars, file name patterns, URLs, error message text — anything
that isn't a PHP identifier.

When spawning subagents for PHP codebase exploration, instruct them to use LSP tools.
...
PHP implementation choices. The **phpunit** skill governs testing conventions — both are
active simultaneously when writing PHP tests.

**Strict types everywhere. Types on everything. If PHP can check it at compile time, make
it do so.**
