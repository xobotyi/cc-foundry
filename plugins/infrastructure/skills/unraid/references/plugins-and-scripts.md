...
The gateway to the Unraid plugin and Docker ecosystem. Adds an **Apps** tab to the
WebGUI with an app-store interface.
...
- Action Center: update notifications, deprecated/incompatible app alerts, blacklisted
  apps removed for security or compatibility
...
- **Reinstall**: Apps tab > Previous Apps filter > follow prompts (settings restored
  from saved XML templates at `/boot/config/plugins/dockerMan/templates-user/`)
...
**GPU and hardware:**
- **Nvidia GPU idle script**: kicks GPU into idle power state at bootup. Without this,
  GPUs in CLI mode run at full power until a task uses them
...
- **awesome-unraid-user-scripts** (GitHub): curated collection organized by category
  (array, media, health, backups, Docker, VMs, security, notifications)
...
Alert categories: disk errors, parity issues, temperature warnings, update availability,
backup completion/failure, container health.
