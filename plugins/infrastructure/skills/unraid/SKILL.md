...
- **High-Water** (default): progressively fills disks, good for media servers
- **Most-Free**: spreads across disks, good for throughput
- **Fill-Up**: sequential, good for archives
...
Create custom bridge networks for inter-container DNS resolution and isolation:

```bash
docker network create my-stack
```

Preserve across restarts with a User Script at array start.
