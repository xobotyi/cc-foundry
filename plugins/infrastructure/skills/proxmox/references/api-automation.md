...
Proxmox VE exposes a comprehensive REST API that maps 1:1 to all GUI operations. Every action available in the web
interface is available programmatically.
...
API tokens provide non-interactive authentication without exposing user credentials. Each token has an ID and a secret
...
- **Privilege separation** (`privsep=1`, default): Token permissions are the intersection of the user's permissions and
  the token's explicit permissions. Always use privilege separation for least-privilege access.
- **No privilege separation** (`privsep=0`): Token inherits full user permissions. Avoid for automation — overly broad
  access.
...
Returns a `CSRFPreventionToken` (for PUT/POST/DELETE) and a `PVEAuthCookie`. Tickets expire after 2 hours.
...
- Use `lifecycle { ignore_changes }` for fields that Proxmox modifies outside Terraform (e.g., disk size after manual
  resize)
...
Hookscripts execute at guest lifecycle events (pre-start, post-start, pre-stop, post-stop). They are stored in a
...
Hookscript receives: `$vmid $phase` where phase is `pre-start`, `post-start`, `pre-stop`, or `post-stop`.
...
For automation-heavy workloads with high API request volume, increase the number of worker processes to prevent slow
request handling or timeouts:
...
For clusters using PCI/USB passthrough with HA, define cluster-wide resource mappings to decouple device configuration
from hardware-specific addresses:

```bash
pvesh create /cluster/mapping/pci --id mygpu --map "node=pve1,path=0000:01:00"
```

This assigns a cluster-unique ID to hardware, so HA failovers and automated processes reference the mapping ID rather
than a host-specific PCI address. Proxmox verifies the hardware matches the mapping on each node.
