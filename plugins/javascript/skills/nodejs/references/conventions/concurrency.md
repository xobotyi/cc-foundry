...
Four threads by default, shared by every asynchronous `fs` API, `dns.lookup()`, the asynchronous `crypto` APIs
(`pbkdf2`, `scrypt`, `randomBytes`, `randomFill`, `generateKeyPair`), and every asynchronous `zlib` API.

Saturating it stalls every other user of it, which is why one slow file operation can present as a DNS failure. Raise it
with `UV_THREADPOOL_SIZE` in the process environment. The pool is created during runtime initialization, before user
code runs, so assigning `process.env.UV_THREADPOOL_SIZE` from inside the program does nothing.

## Worker threads

`node:worker_threads` is Stability 2, and each worker carries a full V8 isolate.
...
- **`resourceLimits` on the `Worker` constructor caps one worker**, where `--max-old-space-size` applies per isolate and
  therefore to every worker separately.
...
**A signal with many listeners warns.** `events.setMaxListeners(n, signal)` raises the cap for one signal that many
concurrent operations subscribe to, which is the case a process-wide shutdown signal always reaches.
