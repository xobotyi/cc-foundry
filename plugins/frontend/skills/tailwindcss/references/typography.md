...
Override line-height inline: `text-sm/6`, `text-sm/7`, `text-lg/loose` — sets both size and line-height in one class.
...
`decoration-<color>` — uses theme color palette (same namespace as `text-*`, `bg-*`). Supports opacity modifier:
`decoration-sky-500/30`. Custom: `decoration-(<prop>)`, `decoration-[<value>]`.
...
**Opacity modifier:** `text-shadow-lg/30` — default opacities are low (≤20%), increase for more pronounced effect.

**Shadow color:** `text-shadow-<color>` sets `--tw-text-shadow-color`. Colored shadows default to 100% opacity; use
modifier to reduce: `text-shadow-sky-300/50`.
...
`text-balance` is best for headings — browsers cap it at ~6 lines for performance. `text-pretty` is best for body
paragraphs to prevent single-word last lines.
...
Spaces in arbitrary values: use underscore (`content-['Hello_World']`). Literal underscore: escape it
