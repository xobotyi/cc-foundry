...
Follow this order: (1) reactive state, (2) logic that modifies state, (3) lifecycle hooks for side effects, (4) return
refs.
...
- Defaults with 3.4 and below: `withDefaults(defineProps<Props>(), { count: 0 })`. Mutable reference defaults (arrays,
  objects) must use factory functions with `withDefaults`.
...
- **Props stability** — pass derived booleans (`active="item.id === activeId"`) instead of raw IDs to prevent
  unnecessary child re-renders.
- **`v-once`** — render once, skip all future updates. For truly static content.
- **`v-memo`** — conditionally skip sub-tree updates. Accepts a dependency array; only re-renders when a dependency
  changes. Use on `v-for` lists where most items don't change.
- **Computed stability (3.4+)** — computed only triggers effects when value actually changes. Avoid returning new
  objects from computed (each new object is "different").
...
This skill provides Vue-specific conventions. The coding skill governs workflow; language skills govern JS/TS choices;
this skill governs component architecture, reactivity patterns, and framework API usage.
